...
def analyze():
    ...
    # Calculate the filtered ("clean") sentiment values:
    sentiments[0]['fake_class'] = 2
    sentiments[1]['fake_class'] = 1
...
#     """Configures logging to both file and terminal."""
...
# # Завантаження моделі та токенізатора
# # tokenizer = AutoTokenizer.from_pretrained('nlptown/bert-base-multilingual-uncased-sentiment')tabularisai/multilingual-sentiment-analysis
...
#     if 'product-comments__list-item' in comments:
#         product_name = extract_product_name_from_h2(html_content)
#         print(len(comments['product-comments__list-item']))
#         for text_content in comments['product-comments__list-item']:
#             text_content_with_spaces = add_spaces_to_text(text_content)
#             nickname = extract_nickname(text_content_with_spaces)
#             clean_text = preprocess_text(text_content_with_spaces)
...
#                 fake_class = int(np.argmax(fake_scores[0]))
#                 #clean_text = ''
#                 logger.info({'nickname': nickname, 'text': clean_text, 'sentiment': sentiment,
#                                    'fake_class': fake_class, 'fake_score': fake_scores[0][fake_class]})
...
#     # Convert numpy types in response_data to native Python types
#     converted_response = convert_numpy(response_data)
#     return jsonify(converted_response)
